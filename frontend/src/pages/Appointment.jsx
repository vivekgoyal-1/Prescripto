...
const Appointment = () => {
    ...
    // Force refresh doctors data on component mount
    const refreshDoctorData = async () => {
        try {
            setLoading(true)
            await getDoctosData() // This should update the doctors array in context
        } catch (error) {
    ...
    // Refresh doctor data when component mounts or docId changes
    useEffect(() => {
        refreshDoctorData()
    }, [docId])

    // Fetch specific doctor info after doctors data is updated
    useEffect(() => {
        if (doctors.length > 0) {
            fetchDocInfo()
        }
